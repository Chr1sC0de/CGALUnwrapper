...
namespace monge{
    ...
    VectorPairString surface_mesh(Mesh_3 & mesh,
        ...
        VectorVertexindex_3 mp_vector_vertex_index;

        for (Vertexindex_3 vi: mesh.vertices()){
            mp_vector_vertex_index.push_back(vi);
        }

        #pragma omp parallel for
        for (int i=0; i<mp_vector_vertex_index.size(); i++){
            int counter = 0;
            Vertexindex_3 vi = mp_vector_vertex_index[i];
            VectorPoint_3 neighbour_points(n_nearest_neighbours);
            ...
            for (K_neighbor_search::iterator it=search.begin(); it!=search.end(); it++){
                neighbour_points[counter++] = mesh.point(it->first);
            }
